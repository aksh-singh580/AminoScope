//  MyoglobinSceneView.swift
//  AminoScope
//
//  Created by Prabhas Kumar on 18/12/24.
...
struct MyoglobinSceneView: View {
            ...
            if !showAR {
                NavigationSplitView {
                    ScrollView {
                        HStack(alignment: .top) {
                            VStack(alignment: .leading) {
                                ...
                                Text("Structure")
                                    .foregroundStyle(
                                        mode == "Structure" ? Color.accentColor : Color.primary
                                    )
                                    .fontWeight(
                                        mode == "Structure" ? .medium : .regular
                                    )
                                    .padding(.horizontal, 8)
                                    .frame(width: 288, height: 44, alignment: .leading)
                                    .background(
                                ...
                                Group {
                                    ...
                                    switch mode {
                                        ...
                                        Text(
                                            "Myoglobin is a small, globular oxygen-binding protein predominantly found in muscle tissue, where it serves as an essential oxygen reservoir, ensuring a steady supply of this vital gas during periods of intense activity or low oxygen availability. Structurally, it features a tightly folded polypeptide chain that encloses a heme group containing an iron ion, which directly binds oxygen. Due to its compact shape and high affinity for oxygen, myoglobin can rapidly release its stored supply when muscle cells demand it, aiding in cellular respiration and energy production. Its efficient oxygen storage capacity makes it critical for diving mammals, allowing them to stay submerged for extended periods without surfacing for air."
                                        )
                                        .font(.body)
                                        .foregroundStyle(Color.primary)
                                        ...
                                        Text(
                                            "In myoglobin, the alpha helices, highlighted in blue, create a stable and compact framework. These helices are arranged in a specific pattern that ensures the protein maintains its shape and functionality."
                                        )
                                        .font(.body)
                                        .foregroundStyle(Color.primary)
                                        ...
                                        Text("About Oxygen Binding Pockets")
                                            .font(.headline)
                                            .foregroundStyle(Color.secondary)
                                            .padding(.bottom, 1)
                                        ...
                                        Text(
                                            "In myoglobin, the oxygen binding pockets, highlighted in pink, are strategically positioned within the alpha-helical framework. These specialized sites facilitate the reversible binding and release of oxygen molecules, allowing myoglobin to effectively manage oxygen supply during muscle activity."
                                        )
                                        .font(.body)
                                        .foregroundStyle(Color.primary)
                                        ...
                                        Text(
                                            "In myoglobin, the heme group, highlighted in red, is central to its functionality. It contains an iron atom that directly interacts with oxygen, enabling myoglobin to capture and release oxygen efficiently as needed, playing a crucial role in oxygen transport and storage."
                                        )
                                        .font(.body)
                                        .foregroundStyle(Color.primary)
                    ...
                    ZStack(alignment: .bottomTrailing){
                        Group{
                            if showAR {
                                ARKitView(mode: $mode) // AR view
                                    .ignoresSafeArea(.all)
                                    

                            } else {
                                ModelKitView(mode: $mode) // 3D view
                            }
                        }
...
struct ModelKitView: View {
    ...
    func loadScene() {
        if let loadedScene = SCNScene(named: "myoglobin-model_1.scn") {
            scene = loadedScene
            scene?.background.contents = colorScheme == .dark ? UIColor.darkGray : UIColor.white
            applyColorScheme(to: loadedScene, mode: mode)
        } else {
            // // print("Could not load myoglobin-model_1.scn")
            scene = nil
        }
    }
...
struct ARKitView: UIViewRepresentable {
    ...
    func updateModel(for mode: String, in arView: ARSCNView) {
        ...
        // Load the scene
        guard let scene = SCNScene(named: "myoglobin-model_1.scn") else {
            // // print("Failed to load the model.")
            return
        }
        // Apply any customizations
        applyColorScheme(to: scene, mode: mode)

        // Clone the model node
        let modelNode = scene.rootNode.clone()
        modelNode.name = "MyoglobinModel" // Name the node for identification
    ...
    class Coordinator: NSObject, ARSCNViewDelegate {
        ...
        @objc func handlePinch(_ gesture: UIPinchGestureRecognizer) {
            ...
            guard let modelNode = arView.scene.rootNode.childNode(withName: "MyoglobinModel", recursively: true) else { return }
...
func applyColorScheme(to scene: SCNScene, mode: String) {
    ...
    scene.rootNode.enumerateChildNodes { (node, _) in
        ...
        newData.withUnsafeMutableBytes { (ptr: UnsafeMutableRawBufferPointer) in
            ...
            for i in 0..<vectorCount {
                ...
                if floatColorMatchesOneOf(r, g, b, in: activeColors) {
                ...
                } else if floatColorMatchesOneOf(r, g, b, in: inactiveColors) {
...
func floatColorMatchesOneOf(_ r: Float, _ g: Float, _ b: Float,
